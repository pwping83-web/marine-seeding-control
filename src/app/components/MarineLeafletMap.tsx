...
import type { MarineBasemap, MarineLeafletMapProps } from "./marine-map-shared";
import {
  TILE_ATTR,
  TILE_CARTO_DARK,
  TILE_CARTO_VOYAGER,
} from "./marine-map-shared";
import { MarineKakaoMap } from "./MarineKakaoMap";

export type {
  MarineBasemap,
  MarineLeafletDrop,
  VesselMarkerVariant,
  MarineLeafletMapProps,
} from "./marine-map-shared";
export { TILE_ATTR, TILE_CARTO_DARK, TILE_CARTO_VOYAGER } from "./marine-map-shared";
...
function WaypointEditorMarker({
  ...
  return (
    ...
    >
      <Popup className="route-wpt-popup">
        <div className="w-[9.5rem] space-y-1.5 px-2.5 py-2 text-xs">
...
export function MarineLeafletMap(props: MarineLeafletMapProps) {
  const k = import.meta.env.VITE_KAKAO_JAVASCRIPT_KEY?.trim();
  if (k && !props.offlineNoTiles) {
    return <MarineKakaoMap appkey={k} {...props} />;
  }
  return <MarineLeafletMapImpl {...props} />;
}
