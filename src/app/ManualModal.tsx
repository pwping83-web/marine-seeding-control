...
const SECTIONS: {
  ...
  },
  {
    ...
    content: [
      ...
      {
        heading: "항로 길안내·보고·구역 면적",
        body: "사이드바 **항로 길안내** 줄에서 목적은 **네비게이션 형식의 길안내**입니다.\n① **내비(호 위)** — 위·경도로 경로를 **경로 확정**하거나, 모달에서 「**항로 꼭짓점 찍기**」를 켠 뒤 지도를 클릭해 꼭짓점을 쌓고 마커에서 위·경도를 수정할 수 있습니다. 「**안내 시작**」으로 자동차 내비처럼 거리·방향·음성 안내가 나옵니다.\n② **살포 예정** — 항로가 2점 이상이면 균등 배치로 예정점을 자동 찍거나, 「살포 예정 찍기」로 직접 찍습니다(연두색, 참고용).\n③ **지도 경로** — 금일 항로를 지도에 켜 두면 꼭짓점·선으로 확인합니다.\n④ **보고(맨 오른쪽)** — 출발·경로·복귀·살포·기상 요약·CSV·**PDF 출력**(인쇄 창에서 PDF 저장)입니다.\n살포 점이 3개 이상이면 **구역 추정 면적(ha)** 이 표시됩니다(살포점 외곽 근사, 참고치).\n**현장 상황보고**를 입력·반영하면, 당분간 그 값이 예보 기반 판단에 섞일 수 있습니다.",
      },
      {
        heading: "선박 위치 (사이드바)",
