...
| 함정 `/mobile` | **「살포 시작」** 후 GPS가 잡힌 상태에서 **「센서 시뮬 1건」**을 눌러 **지도에 점·금일 건수**가 맞는지 본다(Supabase를 켠 경우 DB 반영까지). |

이 단계의 통과 기준: **“한 번의 시뮬 기록 액션 = 지도에 점 1개 + (연동 시) `seed_drop_records` 1행 + 이력 목록 1행”** 이 시뮬에서도 동일하게 느껴진다.
...
- **「테스트 살포 1건」**을 누를 때마다:
  ...
  - **라벨**: `YYYY-MM-DD T01` 형태(날짜·공백·`T`·순번)로, 같은 날·같은 GNSS 모드 세션에서 **누른 순서**가 2자리로 붙습니다. (「내 위치 찾기」를 끄면 순번은 다음에 켤 때 `01`부터 다시 시작. 예전 데이터는 `투하-`·`GNSS-`·`센서-` 접두사가 있을 수 있습니다.)
...
|-----------------|-------------------------------------|
| `id` | 고유 ID (문자열, 예: `1005`) |
| `label` | 화면·리포트용 라벨 (예: `2026-05-12 T01`, 구역 `A01` 등) |
...
- **함정 `/mobile`**: **「센서 시뮬 1건」** → 살포 중·위치 수신 전제 하에 동일하게 1건 적재·지도 표시 (코드: `src/app/MobileDeckView.tsx`의 `recordManualSensorDrop`).
