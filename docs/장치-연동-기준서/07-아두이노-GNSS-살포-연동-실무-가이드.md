# 07. 아두이노·GNSS 살포 연동 실무 가이드 (현장 단말 → 관제·DB)

> **문서 분류**: HTTP·JSON 필드·엔드포인트 값은 [02-신호-프로토콜-명세.md](02-신호-프로토콜-명세.md), MCU·GPS·LTE·투하 핀은 [01-기기사양-기준서.md](01-기기사양-기준서.md)·[04-투하-작동-구조.md](04-투하-작동-구조.md)·[06-실제-연동-방법과-문제점.md](06-실제-연동-방법과-문제점.md). **본 07번**은 관제 **웹 버튼 동작**, **`seed_drop_records` 필드 대응**, **`telemetry-ingest` 경로**, **BLE·폰 경유 개념**을 한곳에 모은 실무용입니다. (이전 경로: `docs/hardware-arduino-gnss-seed/README.md`)
...
**함께 읽기(현장 스케치·주기)**: [아두이노 폴더 안내](../../arduino/README.md) · [관제 웹 마커·궤적·주기·서버 한도](../../arduino/아두이노-스케치-모음/문서/03-관제웹-마커-궤적-주기-서버-한도.md)
...
- **아두이노·ESP32 → Wi‑Fi/LTE → 서버**: 아래 **7.1(경로 A)**, [해상 궤적·살포 통합 스케치](../../arduino/아두이노-스케치-모음/해상-궤적-살포-통합/esp32-marine-telemetry/) 폴더, Edge **`telemetry-ingest`**, Secret **`DEVICE_INGEST_SECRET`** 순으로 맞춘다.
- **센서 → 거치대 → 블루투스 → 폰 → 서버**: **7.2(경로 B)** 및 [블루투스-살포-거치대-연동-계획.md](../기획-연동-초안/블루투스-살포-거치대-연동-계획.md), [투하-연동-제안서-방안1-방안2.md](../기획-연동-초안/투하-연동-제안서-방안1-방안2.md)를 따른다. (살포 **시작/중지**용 `ship_command_logs`와, **투하 1건**용 `seed_drop_records`는 역할이 다르다 — 7.2 참고.)
...
2. **좌표**: 그 순간의 **GNSS**를 읽어 이 문서 **2절·7절**과 같은 필드(`lat`,`lng`,`recorded_at` 등)로 묶는다.  
...
2. **이 저장소와 맞추려면**  
   ...
   - 아두이노는 **Edge `telemetry-ingest`**(이 문서 **7.1**)에 장비 시크릿을 붙여 POST하는 방식이 이미 스케치에 맞춰져 있습니다. 동일 필드 의미의 JSON을 쓰면 됩니다(세부 URL·헤더는 배포 환경에서만 관리).
...
참고 스케치: `arduino/아두이노-스케치-모음/해상-궤적-살포-통합/esp32-marine-telemetry/esp32_marine_telemetry.ino` — 주석에 **궤적은 `vessel-track-ingest`**, **살포 1건은 `telemetry-ingest`**로 나뉘어 있다고 명시되어 있습니다. **웹과 “직접 소켓 연결”할 항목**은 브라우저가 아니라 **배포된 Edge URL + 시크릿 + JSON 스키마**입니다.
